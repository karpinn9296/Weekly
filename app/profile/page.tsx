...
import { BiArrowBack, BiCalendar, BiTrash, BiLogOut, BiCamera, BiFolder, BiListUl } from "react-icons/bi";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setCustomId(user.email?.split('@')[0] || "user"); 
      ...
      if (userDoc.exists()) {
        ...
        setBio(data.bio || "");
        if (data.customId) setCustomId(data.customId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), { bio, customId, headerUrl, updatedAt: new Date() }, { merge: true });
          ...
          <div style={{ flex: 1, width: '100%', maxWidth: '640px', background: 'white', minHeight: '100vh', boxShadow: '0 0 20px rgba(0,0,0,0.03)' }}>
            ...
            <div style={{ padding: '15px 25px', position: 'relative' }}>
              ...
              <div style={{ marginBottom: '20px', marginTop: '10px', paddingRight: '110px' }}>
                {isEditing ? (
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', gap: '8px', color: '#888', fontSize: '0.85rem' }}>
                       <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                         <BiCalendar /> 가입일: {user.metadata.creationTime ? format(new Date(user.metadata.creationTime), "yyyy년 M월") : "알 수 없음"}
                       </span>
                    </div>
                  </>
                )}
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid #eee', marginTop: '10px' }}>
                 <div 
                   className="tab-item"
                   onClick={() => { setActiveTab('all'); setSelectedWeekFilter(null); }}
                   style={{ 
                     borderBottom: activeTab === 'all' ? '3px solid #333' : '3px solid transparent',
                     color: activeTab === 'all' ? '#333' : '#888'
                   }}
                 >
                   <BiListUl size={20} /> 
                   <span>전체 글</span>
                 ...
                 <div 
                   className="tab-item"
                   onClick={() => { setActiveTab('weekly'); setSelectedWeekFilter(null); }}
                   style={{ 
                     borderBottom: activeTab === 'weekly' ? '3px solid #333' : '3px solid transparent',
                     color: activeTab === 'weekly' ? '#333' : '#888'
                   }}
                 >
                   <BiFolder size={20} /> 
                   <span>주차별 기록</span>
            ...
            <div style={{ padding: '20px', backgroundColor: '#f9f9f9', minHeight: '300px' }}>
              ...
              {loading && <p style={{ padding: '20px', textAlign: 'center', color: '#888' }}>로딩 중...</p>}
              ...
              {!loading && activeTab === 'weekly' && !selectedWeekFilter && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px' }}>
                  {weeks.map(week => (
                    <div 
                      key={week.id} 
                      onClick={() => setSelectedWeekFilter(week.id)}
                      style={{ 
                        backgroundColor: 'white', padding: '20px', borderRadius: '16px', cursor: 'pointer',
                        border: '1px solid #eee', boxShadow: '0 2px 5px rgba(0,0,0,0.03)',
                ...
                <>
                  {selectedWeekFilter && (
                    <button 
                      ...
                      style={{ marginBottom: '15px', background: 'none', border: 'none', color: '#1d9bf0', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center' }}
                    >
                        ...
                        <div style={{ display: 'flex', gap: '15px' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                              <span style={{ fontWeight: 'bold', fontSize: '1rem', color: '#333' }}>{name}</span>
