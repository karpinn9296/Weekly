...
export default function UserProfilePage() {
  ...
  const weeks = useMemo(() => {
    ...
    return uniqueWeeks.map(weekId => {
      ...
      return { id: weekId, label: post?.weekLabel || weekId, count: myPosts.filter(p => p.weekId === weekId).length };
    });
  }, [myPosts]);
  ...
  const onFileSelect = (e: React.ChangeEvent<HTMLInputElement>, type: 'avatar' | 'header') => {
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      reader.addEventListener('load', () => { setCropImageSrc(reader.result as string); setCropType(type); setShowCropper(true); });
  ...
  const onCropComplete = (croppedBlob: Blob) => {
    ...
    if (cropType === 'avatar') { setAvatarBlob(croppedBlob); setAvatarPreview(previewUrl); } else { setHeaderBlob(croppedBlob); setHeaderPreview(previewUrl); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (avatarBlob) { const avatarRef = ref(storage, `profiles/${user.uid}/avatar_${Date.now()}`); await uploadBytes(avatarRef, avatarBlob); photoURL = await getDownloadURL(avatarRef); }
      if (headerBlob) { const headerRef = ref(storage, `profiles/${user.uid}/header_${Date.now()}`); await uploadBytes(headerRef, headerBlob); headerUrl = await getDownloadURL(headerRef); }

      const batch = writeBatch(db);
      const userRef = doc(db, "users", user.uid);
      const updateData: any = { displayName: name, photoURL: photoURL, bio, customId, headerUrl, updatedAt: new Date() };
      ...
      postsSnap.forEach((postDoc) => { batch.update(postDoc.ref, { authorName: name, authorPhoto: photoURL, authorIsAdmin: isAdmin }); });
      await batch.commit();
      setProfileData({ ...profileData, ...updateData });
      setIsEditing(false);
      setMyPosts(prev => prev.map(p => ({ ...p, authorName: name, authorPhoto: photoURL, authorIsAdmin: isAdmin })));
  ...
  const displayPosts = activeTab === 'all' ? myPosts : (selectedWeekFilter ? myPosts.filter(p => p.weekId === selectedWeekFilter) : []);
        ...
        <div className="mobile-container" style={{ display: 'flex', width: '100%', maxWidth: '1200px', alignItems: 'flex-start' }.alignItems ? { display: 'flex', width: '100%', maxWidth: '1200px' } : { display: 'flex', width: '100%', maxWidth: '1200px' }}>
          ...
          <div style={{ flex: 1, width: '100%', maxWidth: '640px', background: 'white', minHeight: '100vh', boxShadow: '0 0 20px rgba(0,0,0,0.03)' }}>
            ...
            <div className="profile-header">
              <button className="mobile-only" onClick={() => router.push("/")} style={{ position: 'absolute', top: '15px', left: '15px', zIndex: 50, width: '36px', height: '36px', borderRadius: '50%', border: 'none', backgroundColor: 'rgba(0,0,0,0.5)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backdropFilter: 'blur(4px)' }}>
                <BiArrowBack size={20} />
              </button>
              ...
              <div style={{ position: 'absolute', bottom: '20px', left: '20px', width: '120px', height: '120px', borderRadius: '50%', border: '4px solid white', background: 'white', overflow: 'hidden', boxShadow: '0 4px 15px rgba(0,0,0,0.2)', zIndex: 10 }}>
            ...
            <div style={{ padding: '15px 25px', position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', top: '15px', right: '25px', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 5 }}>
                  {isEditing ? (
                    ...
                    <>
                      ...
                      <button onClick={() => setIsEditing(true)} style={{ padding: '7px 15px', borderRadius: '20px', border: '1px solid #ddd', background: 'white', fontWeight: 'bold', cursor: 'pointer', color: '#333', fontSize: '0.9rem' }}>프로필 수정</button>
                    </>
                  )}
                </div>
              )}
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid #eee', marginTop: '10px' }}>
                 <div className="tab-item" onClick={() => { setActiveTab('all'); setSelectedWeekFilter(null); }} style={{ borderBottom: activeTab === 'all' ? '3px solid #333' : '3px solid transparent', color: activeTab === 'all' ? '#333' : '#888' }}><BiListUl size={20} /> <span>전체 로그</span></div>
                 <div className="tab-item" onClick={() => { setActiveTab('weekly'); setSelectedWeekFilter(null); }} style={{ borderBottom: activeTab === 'weekly' ? '3px solid #333' : '3px solid transparent', color: activeTab === 'weekly' ? '#333' : '#888' }}><BiFolder size={20} /> <span>주차별 로그</span></div>
              </div>
            </div>
